...
% Gemmini dialects comparison
\begin{frame}{Gemmini vs GemminiLow: Operations and Purpose}
  \scriptsize
  \begin{tabularx}{\textwidth}{@{}lXXX@{}}
    \toprule
    Layer & Purpose & Example ops & When used in pipeline \\
    \midrule
    Gemmini (high-level) & Express accelerator-friendly math with tensor-like operands; preserves
      some abstraction for scheduling and tiling. & \code{gemmini.matmul}, \code{gemmini.conv}, \code{gemmini.config} & Emitted by \pathcode{Conversion/ONNXToGemmini/ONNXToGemmini.cpp} when pattern matches; input to tiling pass. \\
    GemminiLow (low-level) & Explicit hardware commands and small operands (tiles), representing
      individual scratchpad mvin/matmul/mvout sequences suitable for LLVM lowering. & \code{gemmini_low.mvin}, \code{gemmini_low.matmul}, \code{gemmini_low.mvout}, \code{gemmini_low.fence} & Produced by \code{createGemminiToGemminiLowPass()} and tiled by \pathcode{Transform/Gemmini/GemminiTiling.cpp}; consumed by scratchpad allocation and LLVM lowering. \\
    \bottomrule
  \end{tabularx}
  \vspace{0.5em}
  {\tiny Notes: Gemmini ops carry higher-level attributes (dataflow, accumulate/identity), while GemminiLow ops record concrete `spad_offset_rows` and `acc_offset_rows` used by \pathcode{Transform/GemminiLow/StaticScratchpadAllocation.cpp}.}
\end{frame}

% OMTensor layout diagram
\begin{frame}{OMTensor Descriptor Memory Layout}
  \scriptsize
  \begin{center}
    \begin{tikzpicture}[node distance=0.4cm, font=\scriptsize]
      \node[draw,rectangle,minimum width=10.5cm,minimum height=1.1cm,fill=ieeeblue!6] (box) {};
      \node[anchor=west] at (-4.9,0.2) {\textbf{OMTensor*} struct memory (conceptual)};
      \draw (-4.8,-0.15) rectangle (-1.8,0.6) node[pos=0.5]{\small pointer to C struct};
      \draw (-1.7,-0.15) rectangle (1.0,0.6) node[pos=0.5]{\small `data_ptr` (raw data buffer)};
      \draw (1.1,-0.15) rectangle (3.2,0.6) node[pos=0.5]{\small `allocated_ptr` (optional)`};
      \draw (3.3,-0.15) rectangle (5.0,0.6) node[pos=0.5]{\small `shape*` pointer};
      \draw (5.1,-0.15) rectangle (6.0,0.6) node[pos=0.5]{\small `rank`};
      \draw (6.1,-0.15) rectangle (7.8,0.6) node[pos=0.5]{\small `dtype`};
      \node[below=1.0cm of box,align=left] {\tiny See \pathcode{include/onnx-mlir/Runtime/OMTensor.h} for definitions and API: constructors, \code{omTensorGetDataPtr}, \code{omTensorGetShape}, and ownership semantics.};
    \end{tikzpicture}
  \end{center}
\end{frame}

% 10b
\begin{frame}{Fallback: Which Ops, How the Decision Is Made, and Performance}
  \scriptsize
  \begin{itemize}
    \item \textbf{Which ops fall back:} any ONNX op that does not meet the Gemmini
      lowering predicates remains on the standard ONNX-MLIR → Krnl → LLVM path.
      Typical fallbacks: dynamic-shape tensors, unsupported dtypes (non-i8/i32
      quantized shapes for direct lowering), grouped convolutions, odd kernel
      shapes/strides/pads, or ops without a Gemmini pattern.
    \item \textbf{Where the decision lives:} predicate functions in
      \pathcode{src/Accelerators/Gemmini/Conversion/ONNXToGemmini/ONNXToGemmini.cpp}
      (examples: \code{isSupportedGemminiQLinearConv}, \code{isSupportedGemminiMatMulInteger}, \code{isF32StaticTensor}).
    \item \textbf{Runtime fallbacks:} some runtime helpers (\pathcode{src/Accelerators/Gemmini/Runtime/OMRuntimeGemmini.cpp}) include both a hardware dispatch path and a CPU scalar "reference" implementation. The low-level ABI header \pathcode{src/Accelerators/Gemmini/Runtime/gemmini-hardware-abi/include/gemmini.h} also documents CPU fallbacks.
    \item \textbf{Performance implications:}
      \begin{itemize}
        \item Gemmini path: cost = quantize/pack + `mvin` (DRAM→spad) + tiled compute + `mvout` (spad→DRAM). Great speedups for large, tiled ops that amortize mvin/mvout.
        \item Fallback (CPU): cost = scalar loops emitted via Krnl/LLVM or runtime scalar helpers — much slower for large matmuls/convs, but lower overhead for tiny tensors.
        \item Edge cases: small shapes, irregular padding, or non-unit groups often perform better on CPU fallback due to Gemmini dispatch overhead.
      \end{itemize}
    \item \textbf{Where tiling/placement matters:} the tiling pass (\pathcode{src/Accelerators/Gemmini/Transform/Gemmini/GemminiTiling.cpp}) enforces DIM×DIM tiles (\code{GemminiTargetInfo::dim}) and static scratchpad allocation (\pathcode{src/Accelerators/Gemmini/Transform/GemminiLow/StaticScratchpadAllocation.cpp}), which both affect whether a shape benefits from acceleration.
  \end{itemize}
  \vspace{0.5em}
  {\tiny For exact numbers and tradeoffs we will run ResNet-18 benchmarks (see task 17); small microbenchmarks usually show 5–50× speedups for well-tiled matmul/conv, but results vary widely with tile size, memory bandwidth, and quantization overhead.}
\end{frame}


% Convolution memory example
\begin{frame}{Convolution: ONNX Runtime vs ONNX-MLIR Runtime (memory example)}
  \scriptsize
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{block}{ONNX Runtime (library-call model)}
        - Tensors: contiguous host buffers (float32 pointers) with separate shape metadata.\\
        - Convolution executed by a library/kernel that reads/writes host memory directly.\\
        - No compiler-generated ABI glue: model invokes library kernels with raw pointers.
      \end{block}

      \begin{lstlisting}
// simplified: direct convolution kernel (pseudo-C)
void conv_naive(float *out, const float *x, const float *w,
                int N,int C,int H,int W,int K,int R,int S){
  for(n...) for(k...) for(h...) for(w...)
    for(c...) for(r...) for(s...)
      out[idx] += x[xidx] * w[widx];
}
      \end{lstlisting}
    \end{column}

    \begin{column}{0.48\textwidth}
      \begin{block}{ONNX-MLIR Runtime + Gemmini (compiler path)}
        - Generated model code uses `OMTensor` descriptors (pointer, rank, shape[], data ptr).\\
        - Calls are lowered to runtime helpers (e.g., \code{om_gemmini_conv_f32}) which prepare data for Gemmini.
      \end{block}

      \begin{lstlisting}
// Generated by compiler (pseudo-C)
OMTensor *x = omTensorFromBuffer(xbuf, /*shape*/{1,C,H,W}, 4);
OMTensor *w = omTensorFromBuffer(wbuf, /*shape*/{K,C,R,S}, 4);
OMTensor *out = omTensorAlloc(...);
om_gemmini_conv_f32(out, x, w, stride, pad);
      \end{lstlisting}

      \vspace{0.4em}
      {\tiny \textbf{What the runtime does (high level):} allocate OMTensor descriptors → (optional) quantize/pack to i8 → issue Gemmini `mvin` commands to scratchpad → dispatch Gemmini `matmul`/`conv` via RoCC → `mvout` results back → dequantize if needed.}
    \end{column}
  \end{columns}
\end{frame}
...
% ABI definition inserted early for clarity
\begin{frame}{Gemmini ABI: Definition and Contract}
  \scriptsize
  \begin{itemize}
    \item The Gemmini ABI is the C-level contract between generated model code and the runtime/Gemmini helper library.
    \item Public runtime entry points: declared in \pathcode{src/Accelerators/Gemmini/Runtime/OMRuntimeGemmini.hpp} and implemented in \pathcode{src/Accelerators/Gemmini/Runtime/OMRuntimeGemmini.cpp}.
    \item Gemmini hardware ABI macros and helper inline assembly live in \pathcode{src/Accelerators/Gemmini/Runtime/gemmini-hardware-abi/include/gemmini.h}.
    \item Key calling conventions:
      \begin{itemize}
        \item Runtime uses `OMTensor*` descriptors for tensors (pointer, rank, shape, data pointer).
        \item Gemmini RoCC commands are emitted through `ROCC_INSTRUCTION_*` macros which pack arguments into `rs1`/`rs2` registers.
        \item The runtime performs tiling/packing then issues Gemmini commands (CONFIG, MVIN, COMPUTE, MVOUT) according to `gemmini.h` macros.
      \end{itemize}
    \item See \pathcode{src/Accelerators/Gemmini/Runtime/gemmini-hardware-abi/include/gemmini.h} for the exact macro names and packing formats used by the compiler's inline-asm emission.
  \end{itemize}
\end{frame}

% Quantization support slide
\begin{frame}{Quantization Support and Data Paths}
  \scriptsize
  \begin{itemize}
    \item ONNX-MLIR supports quantized paths that target Gemmini's efficient int8 systolic array.
    \item Conversion path: \code{ONNXToGemmini.cpp} detects QLinearConv / MatMulInteger patterns and lowers them to Gemmini-backed calls when supported.
    \item Runtime path: quantized helpers (`om_gemmini_qlinearconv_i8`, `om_gemmini_matmulinteger_i8i8acc32`) are implemented in \pathcode{src/Accelerators/Gemmini/Runtime/OMRuntimeGemmini.cpp}.
    \item Data flow for quantized conv/matmul:
      \begin{enumerate}
        \item Quantize f32 inputs (if dynamic path) or use i8 inputs as-is.
        \item Repack weights to Gemmini layout (e.g., HWOI) using `packOIHWtoHWOI` helper.
        \item Call tiled Gemmini helpers (`tiled_matmul_auto` / `tiled_conv_auto`) which emit RoCC commands.
        \item Dequantize accumulators to f32 if needed.
      \end{enumerate}
    \item Files to inspect: \pathcode{src/Accelerators/Gemmini/Conversion/ONNXToGemmini/ONNXToGemmini.cpp}, \pathcode{src/Accelerators/Gemmini/Runtime/OMRuntimeGemmini.cpp}.
  \end{itemize}
\end{frame}

% Hardware params mapping slide
\begin{frame}{Map: Hardware Parameters → Source Files}
  \scriptsize
  \begin{tabularx}{\textwidth}{@{}lX@{}}
    \toprule
    Hardware Parameter & Source / Location \\
    \midrule
    Array dimension (`DIM`) & \pathcode{src/Accelerators/Gemmini/Support/GemminiTargetInfo.hpp} and generated \code{gemmini_params.h} in \code{gemmini-hardware-abi/include} \\
    Scratchpad rows & \code{GemminiTargetInfo::scratchpadRows()} in \pathcode{Support/GemminiTargetInfo.hpp} and \pathcode{gemmini-hardware-abi/include/gemmini_params.h} \\
    Accumulator rows & \code{GemminiTargetInfo::accRows()} (same files) \\
    Data types (`elem_t`, `acc_t`) & \pathcode{gemmini-hardware-abi/include/gemmini_params.h} \\
    Inline-asm packing formats & \pathcode{src/Accelerators/Gemmini/Conversion/GemminiLowToLLVM/GemminiLowToLLVM.cpp} and \pathcode{gemmini-hardware-abi/include/gemmini.h} \\
    \bottomrule
  \end{tabularx}
  \vspace{0.4em}
  \tiny
  These mappings help you find where a hardware parameter is read or enforced by passes (tiler, allocation) and where matching ABI headers assert expected sizes/typedefs.
\end{frame}
...
\begin{frame}{Troubleshooting Checklist}
  \scriptsize
  \begin{itemize}
    \item Undefined runtime symbols after linking: ensure \code{libgemmini_f32_rv64.a} and \code{libcruntime_rv64.a} are provided and in correct link order (object files before archives).
    \item No \code{.insn} in disassembly: check that runtime was compiled for RISC-V and that Gemmini ABI headers were present during runtime build.
    \item Scratchpad allocation failure: reduce tile size (\code{DIM}) or inspect \pathcode{StaticScratchpadAllocation.cpp} for allocation policy limits.
    \item Wrong weight layout: verify repacking helper `packOIHWtoHWOI` was called and that OMTensor weight pointers match emitted global symbols.
    \item Spike/Simulator mismatch: ensure Spike is built with Gemmini extension and that `--extension=gemmini` is used.
  \end{itemize}
  \vspace{0.4em}
  \begin{lstlisting}[language=bash]
# Useful quick checks
riscv64-linux-gnu-nm -u resnet18.o | grep om_gemmini_
riscv64-linux-gnu-nm -S libgemmini_f32_rv64.a | grep om_gemmini_
riscv64-linux-gnu-objdump -d resnet18_rv64 | grep -E '\.insn|om_gemmini_'
  \end{lstlisting}
\end{frame}
...
\begin{frame}{How ONNX Moves to FPGA Execution}
  ...
  \begin{minipage}{0.94\textwidth}
    ...
    \\[0.4em]
    {\scriptsize\textbf{Step-by-step walkthrough (maps arrows to files and passes):}}
    \vspace{0.15em}
    {\scriptsize\begin{enumerate}
      \item \textbf{Green (compiler lowering):} selection and lowering are implemented in
        \pathcode{src/Accelerators/Gemmini/Conversion/ONNXToGemmini/ONNXToGemmini.cpp} (matches supported ONNX ops and emits either \code{krnl.call} to runtime helpers or high-level `gemmini.*` ops). Follow-up Gemmini passes are declared in \pathcode{src/Accelerators/Gemmini/Pass/GemminiPasses.hpp} and include:
        \begin{itemize}\item \code{createGemminiTilingPass()} → \pathcode{src/Accelerators/Gemmini/Transform/Gemmini/GemminiTiling.cpp}
        \item \code{createGemminiToGemminiLowPass()} → lowers Gemmini → GemminiLow
        \item \code{createStaticScratchpadAllocationPass()} → \pathcode{src/Accelerators/Gemmini/Transform/GemminiLow/StaticScratchpadAllocation.cpp}
        \item \code{createGemminiLowRewritePass()} and \code{createGemminiLowToLLVMPass()} → lower to LLVM with RoCC/inline assembly.
        \end{itemize}
      \item \textbf{Blue (binary build/link):} LLVM dialect emitted by the final lowering is converted to LLVM IR and toolchain steps (`llc`, linker) produce the RV64 ELF; the generated model and runtime implementations (e.g., \pathcode{src/Accelerators/Gemmini/Runtime/OMRuntimeGemmini.cpp}) are linked together with ABI headers (\pathcode{src/Accelerators/Gemmini/Runtime/gemmini-hardware-abi/include/gemmini.h}).
      \item \textbf{Orange (runtime / ABI):} model calls become C ABI calls declared in \pathcode{src/Accelerators/Gemmini/Runtime/OMRuntimeGemmini.hpp} (e.g., \code{om_gemmini_conv_f32}) and implemented in the runtime source; the ABI macros and RoCC field encodings are in \pathcode{src/Accelerators/Gemmini/Runtime/gemmini-hardware-abi/include/gemmini.h}.
      \item \textbf{Red (hardware):} the RoCC-expanded Gemmini commands are executed by the RISC-V core and Gemmini controllers; hardware parameters used by compiler decisions live in \pathcode{src/Accelerators/Gemmini/Support/GemminiTargetInfo.hpp} (mesh dimension, scratchpad rows, accumulator rows) and are referenced throughout the passes above.
    \end{enumerate}}
  \end{minipage}
\end{frame}
...
% Condensed Hailo comparison (replaces multiple Hailo-specific slides)
\begin{frame}{Hailo vs ONNX-MLIR — Condensed Comparison}
  \scriptsize
  \begin{itemize}
    \item Both use ahead-of-time compilation: Hailo emits hardware-specific HEF artifacts; ONNX-MLIR emits native ELF/shared objects for RISC-V + Gemmini.
    \item Key differences: Hailo maps graphs to a spatial dataflow substrate and uses vendor runtime (HailoRT); ONNX-MLIR lowers to MLIR/LLVM and links against Gemmini runtime + ABI.
    \item Practical points: Hailo commonly expects NHWC + int8; ONNX-MLIR defaults to NCHW + float32 unless quantized. Both avoid interpreting the ONNX graph at inference time.
    \item Developer takeaway: keep Hailo slides as illustrative background; focus remaining talk on ONNX-MLIR/Gemmini implementation details and code-paths.
  \end{itemize}
\end{frame}

% TableGen before/after examples
\begin{frame}[fragile]{TableGen: Before and After}
  \scriptsize
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{block}{TableGen input (example)}
\begin{lstlisting}
def GemminiInst <  ... > {
  let dag = { ... };
  let pseudo = "gemmini.mvin $rs1, $rd";
}
\end{lstlisting}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Generated C++ (snippet)}
\begin{lstlisting}
// Auto-generated: instruction decoder
case GemminiInst: {
  uint64_t rs1 = extractField(insn, ...);
  emitMvin(rs1, rd);
  break;
}
\end{lstlisting}
  ...
  \end{columns}
  \vspace{0.3em}
  {\tiny TableGen reduces repetitive decoder and instruction descriptions into compact declarative inputs, producing consistent C++/LLVM tables used by the backend.}
\end{frame}
...
% 15b (simplified)
\begin{frame}{How the Gemmini Backend Is Organized (overview)}
  \small
  \begin{columns}
    \begin{column}{0.48\textwidth}
      \textbf{Entry and Options}
      \begin{itemize}
        \item \code{Accelerator.cpp / InitAccelerators.cpp}: registers Gemmini.
        \item \code{Compiler/}: maps CLI/build flags to a concrete pass pipeline.
        \item Purpose: decide when Gemmini is enabled and which passes to run.
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{Lowering and Transforms}
      \begin{itemize}
        \item \code{Conversion/}: ONNX → Gemmini → GemminiLow → LLVM.
        \item \code{Transform/}: tiling, static scratchpad allocation, low-level rewrites.
        \item \code{Dialect/}: Gemmini and GemminiLow op definitions (TableGen + C++ helpers).
      \end{itemize}
    \end{column}
  \end{columns}

  \vspace{0.6em}
  \textbf{Runtime and Support}
  \begin{itemize}
    \item \code{Runtime/}: C/C++ runtime helpers, OMTensor glue, and calls into \code{gemmini.h} (ABI macros).
    \item \code{Support/}: hardware target parameters (\code{GemminiTargetInfo}) used by tiling and allocation.
    \item These parts are the hardware boundary: runtime links into the ELF and emits RoCC/Gemmini instructions.
  \end{itemize}

  \vspace{0.4em}
  \scriptsize
  Read as responsibilities: top-level registration & options → lowering & transform pipeline → runtime/ABI boundary.
\end{frame}
...
% Static linking details slide
\begin{frame}{Static Linking: Libraries and Approximate Sizes}
  \scriptsize
  \begin{itemize}
    \item Typical link command for the ResNet-18 test (RISC-V toolchain):
      \begin{lstlisting}[language=bash]
g++ -static -o resnet18_rv64 \
  ...
  -lm
    ...
    \item Key archives:
      \begin{itemize}
        \item \code{libgemmini_f32_rv64.a}: Gemmini runtime implementations (\code{OMRuntimeGemmini.o}).
        \item \code{libcruntime_rv64.a}: ONNX-MLIR runtime helpers (\code{OMTensor}, utilities).
        \item Test runner: provides \code{main()} and test harness objects.
      \end{itemize}
    \item Typical ELF section sizes (ResNet-18 example):
      \begin{itemize}
        \item \texttt{.text}: ~1.55 MB (CPU + runtime code)
        \item \texttt{.rodata}: ~46.9 MB (model weights / constants)
        \item \texttt{.data/.bss}: small (tens of KB)
      \end{itemize}
    \item What this means:
      \begin{itemize}
        \item Static linking pulls needed object code out of archives and places it into the final ELF.
        \item Large read-only weights dominate binary size; consider compression, shared storage, or dynamic loading for deployment.
        \item Link order matters: put the object files that reference library symbols before the archives that satisfy them.
      \end{itemize}
    \item See \pathcode{docs/gemmini/ONNX_to_RISCV_ResNet18_Gemmini_Pipeline.tex} for the full reproducible link commands and size reports.
  \end{itemize}
\end{frame}
...
% .insn decoding step-by-step
\begin{frame}{Decode a Gemmini \code{.insn} — Step by step}
  \scriptsize
  \begin{enumerate}
    \item Start with disassembly line: e.g. \code{181be:  00e8b07b  .insn 4, 0x00e8b07b}
    \item Low 7 bits (opcode) = \code{0x7b} → RISC-V \code{custom-3} space (Gemmini uses custom-3). Refer to \pathcode{src/Accelerators/Gemmini/Conversion/GemminiLowToLLVM/GemminiLowToLLVM.cpp} which emits \code{.insn r CUSTOM_3,...}.
    \item Extract the Gemmini "funct" selector (the assembler inline-asm's third numeric argument). Common values used by the lowering:
      \begin{itemize}
        \item \code{0}: CONFIG (config/mode)
        \item \code{2}: MVIN (data movement DRAM→spad)
        \item \code{3}: MVOUT (spad/accum→DRAM)
        \item \code{4}: COMPUTE_PRELOADED (matmul identity/compute)
        \item \code{5}: COMPUTE_ACCUMULATED (matmul accumulate)
        \item \code{6}: PRELOAD (stage accumulator)
      \end{itemize}
    \item The registers printed after ``.insn'' correspond to RISC-V register operands: \code{rd}, \code{rs1}, \code{rs2}. Gemmini packs command metadata into the 64-bit values placed in \code{rs1} and \code{rs2} by the compiler (see packing in \pathcode{src/Accelerators/Gemmini/Conversion/GemminiLowToLLVM/GemminiLowToLLVM.cpp}).
    \item Example mapping (from lowering code): for MVIN (funct=2) the lowering first emits a CONFIG_MVIN with packed \code{rs1} containing \code{mvin_scale<<32 | 0x01} and \code{rs2}=row stride, then emits the MVIN with \code{rs1}=pointer and \code{rs2}=packed(tile_rows<<(32+16) | tile_cols<<32 | spad_offset_rows).
    \item To decode a specific instruction from hex: use the linked object's disasm and the lowering patterns above to identify which funct and which packed layout apply, then extract fields like tile rows/cols/offset by reversing the bit packing (shift/mask operations).
  \end{enumerate}

  \vspace{0.4em}
  {\tiny Tip: search for the inline-asm templates in \pathcode{src/Accelerators/Gemmini/Conversion/GemminiLowToLLVM/GemminiLowToLLVM.cpp} to see the exact packing and reproduce the bit-field extraction.}
\end{frame}
...
% --- Insert: ResNet-18 tiling and scratchpad slides ---
\begin{frame}{Tiling Parameters for ResNet-18 (concrete)}
  \scriptsize
  \begin{itemize}
    \item Gemmini hardware tile dimension: \code{DIM = GemminiTargetInfo::dim} (default in repo: 16). See \pathcode{src/Accelerators/Gemmini/Support/GemminiTargetInfo.hpp}.
    \item ResNet-18 effective shapes: many Conv layers map to matrices that are tiled into 16×16 blocks by \code{GemminiTiling} (\pathcode{src/Accelerators/Gemmini/Transform/Gemmini/GemminiTiling.cpp}).
    \item Tiling rules used in the pass:
      \begin{itemize}
        \item Loop over M, N, K in steps of \code{DIM} (tile rows/cols/depth).
        \item Edge tiles are padded to full \code{DIM} shape (zero-fill) so downstream low-level lowering can assume full tiles.
        \item K alternation uses two buffer slots (double-buffer pattern) to stream tiles into scratchpad; the pass toggles between buffer slots to avoid overwrite hazards.
      \end{itemize}
    \item Why \code{DIM=16}: matches Gemmini systolic mesh sizing and gives good utilization for common ResNet-18 channel counts (multiples or near-multiples of 16 like 64,128). Balances compute density vs scratchpad usage.
    \item Practical implication: shapes whose M/N/K are small or not aligned to 16 may suffer padding overhead and might be better served by CPU fallback for lowest latency.
  \end{itemize}
\end{frame}

\begin{frame}{Scratchpad Allocation Strategy (static pass)}
  \scriptsize
  \begin{itemize}
    \item Static allocation is performed by \code{StaticScratchpadAllocation.cpp} (\pathcode{src/Accelerators/Gemmini/Transform/GemminiLow/StaticScratchpadAllocation.cpp}).
    \item Key policy points:
      \begin{itemize}
        \item Alternating A/B assignment: incoming A/B mvins are assigned alternating base row offsets (e.g. \code{nextSpadA}, \code{nextSpadB}) to avoid conflicts and simplify scheduling.
        \item Accumulator groups: matmuls between fences form a group assigned a single accumulator row (\code{groupFirstAcc}).
        \item Linear-scan allocator: the pass walks GemminiLow ops in program order, assigning concrete \code{spad_offset_rows} and \code{acc_offset_rows}; if allocation exceeds hardware limits (\code{scratchpadRows}, \code{accRows} in \code{GemminiTargetInfo}), the pass fails and lowering may fall back or require smaller tiles.
        \item Conservatism: the static policy avoids global register coloring and instead uses deterministic alternating placement to produce reproducible addresses for LLVM lowering.
      \end{itemize}
    \item Watchouts: excessive concurrent tile live ranges or very large tile counts per group can exhaust scratchpad capacity; reduce \code{DIM} or split the work across more runtime calls.
  \end{itemize}
\end{frame}
...
\begin{frame}{Validation Checklist}
  \begin{itemize}
    \item Check the MLIR files after each important pass (ONNX, Krnl, LLVM).
    \item Confirm runtime calls are present in LLVM IR and object files.
    \item Confirm the final ELF is RV64, statically linked, and contains Gemmini runtime symbols.
    \item Confirm disassembly contains Gemmini custom instructions (\code{.insn}).
    \item Compare Spike timing JSON outputs for Gemmini vs non-Gemmini when running simulation.
  \end{itemize}

  \vspace{0.5em}
  \scriptsize
  \begin{lstlisting}[language=bash]
# 1. Generate model artifacts (example)
$ONNX_MLIR --maccel=Gemmini --mtriple=riscv64-unknown-linux-gnu resnet18.onnx -o resnet18_rv64

# 2. Build and link (example order matters)
g++ -static -o resnet18_rv64 \
  float_model_runner.o \
  resnet18.o \
  libgemmini_f32_rv64.a \
...
# 3. Inspect objects & archives
riscv64-linux-gnu-nm -u resnet18.o | grep om_gemmini_conv_f32
riscv64-linux-gnu-ar t libgemmini_f32_rv64.a

# 4. Verify final ELF and sections
file resnet18_rv64
readelf -SW resnet18_rv64 | grep -E '\.text|\.rodata|\.data|\.bss'

# 5. Find runtime calls and custom instructions in disassembly
riscv64-linux-gnu-objdump -d resnet18_rv64 | grep -E '\.insn|om_gemmini_' -n -A2

# 6. Trace any runtime symbol through the pipeline
...
\begin{frame}{ResNet-18 Benchmark Script (automation)}
  \begin{itemize}
    \item I added a runnable helper: \pathcode{docs/gemmini/run_resnet18_benchmark.sh}.
    \item It automates: MLIR generation, optional RISC-V link, disassembly collection, and a Spike run if available.
    \item Run it on a Linux host with `ONNX_MLIR`, RISC-V toolchain, and Spike configured.
  \end{itemize}
  \vspace{0.4em}
  \scriptsize
  \begin{lstlisting}[language=bash]
chmod +x docs/gemmini/run_resnet18_benchmark.sh
docs/gemmini/run_resnet18_benchmark.sh
  \end{lstlisting}
  \vspace{0.4em}
  Outputs: artifact directory `docs/tests/15-resnet18-float/` with `.disasm`, `.nm`, `.sections`, and optional Spike logs.
\end{frame}
